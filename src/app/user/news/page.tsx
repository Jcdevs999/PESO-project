...
const Newspage = () => {
  ...
  const filterData = news?.filter(
    (item: any) =>
      ...
      filterState.dateRange.FilterDate &&
      new Date(item.publishedAt).getDate() ===
        new Date(filterState.dateRange.FilterDate).getDate()
  );
  news;
  useEffect(() => {
  ...
  const filterOtherData = news?.filter((item: any) => item.urlToImage !== null);
  //(news);
  useEffect(() => {
    const fetchData = async () => {
      try {
  ...
  return (
    <div>
      <div className="h-12 p-7 bg-qc-red text-white px-16 flex items-center  font-bold text-xl border-b-2 border-b-black">
        <h2>News</h2>
      </div>
      <div>
        <div className="px-16 pt-10 pb-10 flex flex-col gap-3 font-bold text-xl">
            ...
            <div className="flex py-4 pt-8">
              <div className="w-fit text-xs border-[1px] border-black px-2 py-2 rounded-md">
                <input
                  ...
                  value={
                    filterState.dateRange.FilterDate
                      ? filterState.dateRange.FilterDate.toISOString().slice(
                          0,
                          10
                        )
                      : ""
          ...
          <div className="grid grid-rows-2 grid-flow-col gap-4 items-center">
            {filterOtherData.slice(20, 26).map((data: any, index: number) => (
              <div key={index}>
